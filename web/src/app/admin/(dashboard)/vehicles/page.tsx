        ...
        <ul className="space-y-3">
          {vehicles.map((v) => {
            ...
            return (
              <li
                key={v.id}
                className="rv-surface flex items-stretch overflow-hidden rounded-xl transition hover:-translate-y-0.5 hover:border-sky-200"
              >
                <Link
                  href={`/admin/vehicles/${v.id}/edit`}
                  aria-label={`Editar ${vehicleName} ${v.year}`}
                  className="group flex min-w-0 flex-1 items-center gap-3 px-4 py-3 sm:py-3.5"
                >
                  ...
                  <span className="shrink-0 self-center text-sm font-medium text-sky-700">Editar</span>
                </Link>
